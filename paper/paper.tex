...
% The Lean identifiers below are long and unbreakable; give the line breaker a little
% slack rather than emitting overfull boxes.
...
\]
consists of integers and satisfies $b(n+k)\equiv b(n)\pmod{k}$ for all $n\ge0$, $k\ge1$;
...
the exponents cannot be dropped, and we identify the necessary condition on the constant
terms: in the diagonal case $A=M=\mathrm{id}$ the condition $F(0)G(0)=1$ is necessary, and
we show that it is not sufficient. The main theorem and its four OEIS corollaries have
been formalized in
Lean~4 with Mathlib; the formalization is stated directly in terms of Mathlib's
\texttt{PowerSeries.exp}, and no theorem in it depends on \texttt{sorryAx}. The sharpness
analysis and the two supplementary results of \Cref{sec:notbala} and \Cref{sec:fails} are
proved by hand and checked numerically.
\end{abstract}
...
\begin{equation}\label{eq:scp}
  b(n+k)\equiv b(n) \pmod{k}\qquad\text{for all }n\ge 0,\ k\ge 1 .
\end{equation}
...
\[
  b(n)\;=\;n!\,\coeff{n}\!\left(W(x)\,F(x)^{A(n)}\exp\!\big(x\,G(x)^{M(n)}\big)\right).
\]
Then $b(n)\in\Z$ for all $n$, and $b(n+k)\equiv b(n)\pmod{k}$ for all $n\ge0$, $k\ge1$.
\end{theorem}
...
function is not of the form~\eqref{eq:bala2017}, as we verify in
\Cref{prop:A293013outside}.
...
\Cref{sec:sharp} we show what the hypotheses cost: shift-stability of the exponents
cannot be dropped from a uniform theorem, and in the diagonal case $A=M=\mathrm{id}$ the
constant terms must satisfy $F(0)G(0)=1$ --- a condition that is necessary but, as we
also show, not sufficient.
...
$\ff{n}{i}=0$ for $i>n$. Fix $n\ge0$ and $k\ge1$. We show
...
So shift-stability cannot be discarded from a statement that is meant to hold for all
admissible $F$. It is not, however, necessary instance by instance: if $F=1$ the factor
$F^{A(n)}$ is identically $1$, the value of $A$ never enters \eqref{eq:closed}, and
\eqref{eq:scp} holds for \emph{every} $A$, shift-stable or not --- with $A(n)=2^{n}$ or
$A(n)=n!$ included. The same remark applies to $M$ when $G=1$. What the hypothesis buys
is uniformity in $F$ and $G$, not necessity.

\subsection{What the constant terms must satisfy}\label{sec:sharpconst}

Throughout this subsection $W=1$ and $A=M=\mathrm{id}$, the diagonal case
\eqref{eq:conjA}; the conclusion is specific to that slice, as we note at the end.
Suppose such a $b$ satisfies \eqref{eq:scp}, and write $F_0=F(0)$, $G_0=G(0)$. Taking
$n=0$ in \eqref{eq:closed}, only $i=0$ contributes, so $b(0)=1$. Taking $n=p$ prime,
$\ff{p}{i}\equiv0\pmod p$ for $1\le i\le p$, so only $i=0$ survives modulo $p$, and its
$G$-exponent is $M(p)\,p=p^{2}$:
\[
...
$F_0G_0\equiv1\pmod p$ for every prime $p$, and therefore
\begin{equation}\label{eq:prodone}
  F_0G_0=1 .
\end{equation}
Outside \eqref{eq:prodone} the congruence indeed fails: with $F=2$, $G=1$ one gets
$b(n)=2^{n}$ and $b(3)-b(0)=7\not\equiv0\pmod 3$.

Over $\Z$, \eqref{eq:prodone} leaves two branches, $(F_0,G_0)=(1,1)$ and $(-1,-1)$, and
the second is nonempty. If $F=-\widetilde F$ and $G=-\widetilde G$ with
$\widetilde F,\widetilde G$ \emph{even} series (functions of $x^{2}$) and
$\widetilde F(0)=\widetilde G(0)=1$, then $b_{F,G}=b_{\widetilde F,\widetilde G}$
identically: for odd $n$ the substitution $x\mapsto -x$ fixes $\widetilde F$ and
$\widetilde G$ and converts $\exp(-x\widetilde G^{n})$ into $\exp(x\widetilde G^{n})$.
So in this diagonal case the congruence holds on a strictly larger domain than
$F(0)=G(0)=1$. This enlarges the diagonal case only; we make no claim about the domain of
\Cref{thm:main} for general shift-stable $A,M$.

Condition \eqref{eq:prodone} is, however, \emph{necessary and not sufficient}: the
evenness above is not a convenience of the proof but a real constraint. Take
\[
  F=-1+x,\qquad G=-1,
\]
so that $F_0G_0=1$ while $-F=1-x$ carries an odd term. Then $b$ begins
\[
  1,\;2,\;-1,\;34,\;-15,\;1546,\;\dots
\]
and $b(4)-b(1)=-15-2=-17\not\equiv0\pmod 3$, so \eqref{eq:scp} fails at $n=1$, $k=3$.
The correct statement is therefore: $F_0=G_0=1$ is sufficient (\Cref{thm:main});
\eqref{eq:prodone} is necessary; and on the branch $F_0=G_0=-1$ sufficiency holds for the
even subfamily above but not in general. We do not determine the exact boundary on that
branch. Over the grid $F=-1+f_1x+f_2x^{2}$, $G=-1+g_1x+g_2x^{2}$ with
$f_i,g_i\in\{-2,\dots,2\}$, all $600$ cases carrying an odd term fail; but that is a
finite search, not a proof that every odd term forces failure.

Finally, \eqref{eq:prodone} is a statement about the diagonal slice only. The argument
used $A(p)=p$ and $M(p)=p$; for $A=\mathrm{id}$, $M\equiv1$ it goes through unchanged and
gives the same condition, but for $A\equiv0$ it does not. There the $i=0$ term gives
$b(0)=W(0)$ and $b(p)\equiv W(0)G_0^{\,p^{2}}\equiv W(0)G_0 \pmod p$, so \eqref{eq:scp}
forces $W(0)\big(G_0-1\big)=0$; for $W(0)\ne0$ this says $G_0=1$ and leaves $F$ entirely
unconstrained --- consistent with \Cref{cor:A361281}, where $F$ is arbitrary. The
condition on the constant terms is thus a property of the exponent slice, not of the
theorem as a whole.
...
The same identification applies to the exponent functions themselves: \Cref{def:ss} says
precisely that a shift-stable $A\colon\N\to\N$ has integral difference ratios. So
\Cref{thm:main} can be read as a closure statement --- the property is transported from
the exponents $A,M$ to the sequence $b$ --- and \Cref{lem:ssclosure} is then a special case
of the closure of that class under sum and product, which is~\cite{cgg2015}. We use the
name \emph{shift-stable} only because it is the property of the exponents that the proof
consumes; the underlying notion is not new.
...
We regard the identification of the two as a useful byproduct of this work. It should be
said that~\cite{cgg2015} does not by itself answer the question left open
in~\cite{bala2017}: it characterizes the ambient class of sequences with integral
difference ratios, but makes no comparison between that class and Bala's subclass of
sequences whose exponential generating function factors as \eqref{eq:bala2017}. Settling
Bala's question requires exhibiting a sequence in the former that is not in the latter,
which is what \Cref{prop:A293013outside} below does.

\subsection{The new families are not of Bala's 2017 form}\label{sec:notbala}

\Cref{thm:main} answers the closing question of~\cite{bala2017} only if the sequences it
produces are genuinely outside \eqref{eq:bala2017}. That is easy to certify, by a
denominator obstruction on the logarithmic derivative.

\begin{lemma}\label{lem:obstruction}
Let $b$ be an integer sequence with $b(0)=1$ and let
$B(x)=\sum_{n\ge0}b(n)x^{n}/n!\in\Q[[x]]$. If $B=\widehat F\exp(x\widehat G)$ for some
$\widehat F,\widehat G\in\Z[[x]]$, then $B'/B\in\Z[[x]]$.
\end{lemma}

\begin{proof}
Evaluating at $x=0$ gives $\widehat F(0)=B(0)=b(0)=1$, so $\widehat F$ is a unit in
$\Z[[x]]$ and $\widehat F'/\widehat F\in\Z[[x]]$. Taking logarithmic derivatives of the
product,
\[
  \frac{B'}{B}=\frac{\widehat F'}{\widehat F}+\big(x\widehat G\big)'
             =\frac{\widehat F'}{\widehat F}+\widehat G+x\widehat G',
\]
and each of the three summands lies in $\Z[[x]]$.
\end{proof}

\begin{proposition}\label{prop:A293013outside}
The sequence \oeis{A293013}, $b(n)=n!\coeff{n}\exp\big(x/(1-x)^{n}\big)$, satisfies
\eqref{eq:scp} but its exponential generating function is not of the form
\eqref{eq:bala2017}.
\end{proposition}

\begin{proof}
The congruence is \Cref{thm:main} with $W=F=1$, $G=1/(1-x)$, $A\equiv0$, $M=\mathrm{id}$.
For the second claim, $b$ begins $1,1,5,55,961,24101,818821,\dots$, and an exact
computation of the logarithmic derivative of $B=\sum b(n)x^{n}/n!$ gives
\[
  \frac{B'}{B}=1+4x+21x^{2}+120x^{3}+755x^{4}+5215x^{5}+\tfrac{117271}{3}x^{6}+\cdots,
\]
whose coefficient of $x^{6}$ is not an integer. By \Cref{lem:obstruction}, $B$ admits no
representation $\widehat F\exp(x\widehat G)$ with $\widehat F,\widehat G\in\Z[[x]]$.
\end{proof}

The same test applies to \oeis{A361036}, whose logarithmic derivative already fails
integrality at $x^{4}$, with $\coeff{4}(B'/B)=2777/2$. It does \emph{not} fire for every
instance: for \oeis{A278070} itself ($F=1/(1-x)$, $G=1$, $A=\mathrm{id}$, $M\equiv1$) the
series $B'/B$ is integral as far as we computed, so that sequence may well be of Bala's
form. The claim we need, and the only one we make, is that the family of
\Cref{thm:main} \emph{contains} sequences outside \eqref{eq:bala2017}. Note also that the
obstruction is one-way: $B'/B\in\Z[[x]]$ is necessary for Bala's form, not sufficient,
...
exists. The base case does survive, in the following form. In \eqref{eq:closed} with
$n=k$, every term with $1\le i\le k$ carries the factor $k$, since
$\ff{k}{i}=k(k-1)\cdots(k-i+1)$; the surviving term is $i=0$, which contributes
$\coeff{0}\big(W F^{A(k)}G^{M(k)k}\big)=W(0)$ because $F(0)=G(0)=1$. As $b(0)=W(0)$ too,
this gives $b(k)\equiv b(0)=W(0) \pmod k$, which is \eqref{eq:scp} at $n=0$. In Bala's
setting $W=1$ it reads $b(k)\equiv1\pmod k$.
...
\section{Coefficients in a localization of \texorpdfstring{$\Z$}{Z}}\label{sec:fails}
...
apply. Its OEIS entry records $a(n+k)\equiv a(n)\pmod k$ for all \emph{odd} $k$, and
indeed the unrestricted congruence fails: $a(0)=1$, $a(2)=2$, so
$a(2)-a(0)\not\equiv0\pmod2$.

The hypothesis $G\in\Z[[x]]$ can in fact be dropped almost entirely. Nothing in
\Cref{sec:proof} uses properties of $\Z$ beyond the ability to multiply an integer by a
coefficient: \Cref{lem:tools} is a statement about integers only, \Cref{lem:newton} is the
binomial theorem in $R[[x]]$ for a natural exponent, and each of the three telescoped
terms \eqref{eq:t1}--\eqref{eq:t3} is an integer multiple of $k$ times a coefficient of a
power series. Recording this costs nothing.

\begin{proposition}\label{prop:ring}
Let $R$ be a commutative ring, let $W,F,G\in R[[x]]$ with $F(0)=G(0)=1$, and let
$A,M\colon\N\to\N$ be shift-stable. Put
\[
  B(n)\;=\;\sum_{i=0}^{n}\ff{n}{i}\,\coeff{i}\!\left(W\,F^{A(n)}G^{M(n)(n-i)}\right)\;\in R .
\]
Then $B(n+k)-B(n)\in kR$ for all $n\ge0$ and $k\ge1$.
\end{proposition}

\begin{proof}
Verbatim \Cref{sec:proof}, with $\Z$ replaced by $R$. Every divisibility invoked there ---
\Cref{lem:tools}\ref{it:poly}, \ref{it:divis}, \ref{it:key} and \ref{it:edge} --- is an
assertion about integers, and in each of \eqref{eq:t1}, \eqref{eq:t2} and \eqref{eq:t3}
that integer multiple of $k$ is multiplied by an element of $R$. \Cref{lem:newton} holds
in $R[[x]]$ because $a$ is a natural number and $P(0)=0$.
\end{proof}

Note that \Cref{prop:ring} is stated for the finite sum, not for the exponential. This is
the only real subtlety: for $R=\Z[1/D]$ the series $\exp(xG^{M(n)})$ does \emph{not} lie in
$R[[x]]$, since the denominators $j!$ involve primes not dividing $D$. It must be read in
$\Q[[x]]$, and the membership $B(n)\in R$ then comes from the closed form rather than from
the exponential. With that said, the localized statement follows.

\begin{corollary}\label{cor:localized}
Let $D\ge2$ and let $W,F,G\in\Z[1/D][[x]]$ with $F(0)=G(0)=1$, and let $A,M$ be
shift-stable. Define $b(n)=n!\coeff{n}\big(WF^{A(n)}\exp(xG^{M(n)})\big)$, computed in
$\Q[[x]]$. Then $b(n)\in\Z[1/D]$ and
\[
  b(n+k)-b(n)\in k\,\Z[1/D]\qquad\text{for all }n\ge0,\ k\ge1 .
\]
\end{corollary}

\begin{proof}
\Cref{prop:closed} is an identity in $\Q$, so $b(n)$ equals the sum $B(n)$ of
\Cref{prop:ring} with $R=\Z[1/D]$; apply that proposition. No coprimality between $k$ and
$D$ is needed.
\end{proof}

\begin{corollary}\label{cor:descent}
In the situation of \Cref{cor:localized}, suppose in addition that $b(n)\in\Z$ for all $n$.
Then $b(n+k)\equiv b(n)\pmod k$ in $\Z$ for every $k$ coprime to $D$.
\end{corollary}

\begin{proof}
Write $b(n+k)-b(n)=k\,y$ with $y\in\Z[1/D]$, say $y=a/D^{m}$ with $a\in\Z$. Then
$\big(b(n+k)-b(n)\big)D^{m}=ka$, so $k$ divides $\big(b(n+k)-b(n)\big)D^{m}$; since
$\gcd(k,D)=1$ this forces $k\mid b(n+k)-b(n)$.
\end{proof}

Applying this to \oeis{A000085} --- $W=F=1$, $G=1+x/2$, $M\equiv1$, $D=2$ --- gives
$a(n+k)\equiv a(n)\pmod k$ for every odd $k$, which is exactly the congruence its OEIS
entry records. We stress that this \emph{recovers} a recorded observation and settles
nothing that was open; its interest is that a sequence outside the hypotheses of
\Cref{thm:main} is nevertheless explained by the same argument.

Two remarks on the boundary of \Cref{cor:descent}. The coprimality hypothesis there is
necessary and not an artefact: $a(2)-a(0)=1$ for \oeis{A000085} is not divisible by $2$,
and $2\mid D$. And for $G=1+x/3$ the values leave $\Z$ altogether --- $b(2)=5/3$ --- so
\eqref{eq:scp} is not literally an integer congruence there; the $k=2$ instance, which a
naive integer reading appears to break, is divisible in $\Z[1/3]$, since $b(2)-b(0)=2/3$
and $(2/3)/2=1/3$. An earlier version of this paper presented that instance as a
counterexample to the localized congruence. It is not one, and \Cref{cor:localized} shows
no counterexample exists.

Consistently with that, a search over $D\in\{2,3,4,5,6,8,9,10,12\}$, with $F$ and $G$ drawn
from $\Z[1/D][[x]]$ and the exponent pairs of \Cref{thm:main}, found no failure of
$b(n+k)-b(n)\in k\,\Z[1/D]$ among $7128$ pairs $(n,k)$ with $n+k\le11$, including pairs
with $\gcd(k,D)>1$.

\Cref{prop:ring} and its corollaries are proved by hand and checked numerically; unlike
\Cref{thm:main} they are not part of the Lean development. Formalizing them is the obvious
next step, and the obstacle is not mathematical: the Lean statement of \Cref{prop:closed}
is tied to $\Z$ through its integrality bookkeeping, so a ring-level refactor of that layer
is required first.

\section{Formal verification}\label{sec:lean}

The main theorem and its four OEIS corollaries have been formalized in Lean~4 against
Mathlib; the source is available
...
work also verified \eqref{eq:scp} over $646{,}400$ pairs $(n,k)$ with $n+k\le100$, across
$128$ combinations of structured and randomized choices of $(W,F,G,A,M)$, with no
exception, and confirmed the sharpness statements of \Cref{sec:sharp}. Each figure quoted
in this paper is printed by a script in the \texttt{verification/} directory of the
repository; the sweep above is \texttt{sweep\_congruence.py} and the localization search of
\Cref{sec:fails} is \texttt{probe\_localization.py}. That correspondence is itself checked:
\texttt{check\_figures.py} fails if this paper quotes a figure no script produces, or if a
script stops producing one it quotes.
...
record that made the problem findable. We are grateful to two readers who reviewed an
earlier draft of this paper and of its repository, and who identified an incorrect
sharpness claim --- that $F(0)G(0)=1$ was not merely necessary but sufficient --- together
with the erroneous counterexample discussed in \Cref{sec:fails}; both have been corrected
here.
...
European J. Combin. \textbf{36} (2014), 564--578.
\href{https://doi.org/10.1016/j.ejc.2013.09.009}{doi:10.1016/j.ejc.2013.09.009}.
