...
the exponents cannot be dropped, and we determine what the constant terms must satisfy:
in the diagonal case $A=M=\mathrm{id}$ the condition $F(0)G(0)=1$ is necessary, but it is
not sufficient. The main theorem and its four OEIS corollaries have been formalized in
...
even subfamily above but not in general.
...
repository; the sweep above is \texttt{sweep\_congruence.py} and the localization probe of
\Cref{sec:fails} is \texttt{probe\_localization.py}.
